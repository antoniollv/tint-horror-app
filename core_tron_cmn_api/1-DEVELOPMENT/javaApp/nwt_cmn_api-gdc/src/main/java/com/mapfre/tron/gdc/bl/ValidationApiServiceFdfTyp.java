...
@Service("FdfTyp")
public class ValidationApiServiceFdfTyp implements ValidationApiService {

    protected static final String CMP_VAL = "CMP_VAL";

    protected static final String DSB_ROW = "DSB_ROW";

    protected static final String FRD_TYP_NAM = "FRD_TYP_NAM";

    protected static final String FRD_TYP_VAL = "FRD_TYP_VAL";
    ...
    public List<ValidationError> conceptValidation(DataInExtended body) {
		...
		if (accion.equals(CTrn.ACN_TYP_MDF)) {
		    ...
		    if(!body.getDataOld().get(FRD_TYP_VAL).equals(body.getData().get(FRD_TYP_VAL))) {
				...
				error = ValidationError.builder().code(lvError.getErrIdnVal()).field(lvError.getPrpNam()).value(body.getDataOld().get(FRD_TYP_VAL)).build();
				errors.add(error);
		    }
		...
		// Validamos  si la descripcion del Tipo de Fraude está informado
		if (body.getData().get(FRD_TYP_NAM) == null || body.getData().get(FRD_TYP_NAM).equals("")) {
			codError = new BigDecimal(20003);
			lvError = lvDlTrnErr.getError(codError, body.getUser().getLanguage().toUpperCase(), "FDF", FRD_TYP_NAM, cmpVal);
			error = ValidationError.builder().code(lvError.getErrIdnVal()).field(lvError.getPrpNam()).value(body.getData().get(FRD_TYP_NAM)).build();
			errors.add(error);
		}
