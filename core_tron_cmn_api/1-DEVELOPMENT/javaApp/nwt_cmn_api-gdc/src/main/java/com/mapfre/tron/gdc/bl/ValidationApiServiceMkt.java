...
@Service("Mkt")
public class ValidationApiServiceMkt implements ValidationApiService {

    protected static final String DSB_ROW = "DSB_ROW";

    protected static final String LNG_VAL = "LNG_VAL";

    protected static final String MKT_NAM = "MKT_NAM";
    ...
    public List<ValidationError> conceptValidation(DataInExtended body) {
		...
		if (accion.equals(CTrn.ACN_TYP_MDF)) {
		    ...
		    if(!body.getDataOld().get(MKT_VAL).equals(body.getData().get(MKT_VAL))) {
				...
				lvError = lvDlTrnErr.getError(codError, body.getUser().getLanguage().toUpperCase(), "MKT", MKT_VAL, cmpVal);
		 ...
		 // No se puede modificar el idioma
		    if(!body.getDataOld().get(LNG_VAL).equals(body.getData().get(LNG_VAL))) {
				codError = new BigDecimal(20120);
				lvError = lvDlTrnErr.getError(codError, body.getUser().getLanguage().toUpperCase(), "MKT", LNG_VAL,cmpVal);
		 ...
		 // No se puede modificar fecha de validez
		    if(!body.getDataOld().get(VLD_DAT).equals(body.getData().get(VLD_DAT))) {
				codError = new BigDecimal(20120);
				lvError = lvDlTrnErr.getError(codError, body.getUser().getLanguage().toUpperCase(), "MKT", VLD_DAT,cmpVal);
				error = ValidationError.builder().code(lvError.getErrIdnVal()).field(lvError.getPrpNam()).value(body.getDataOld().get(VLD_DAT)).build();
				errors.add(error);
		    }
		}
		
		// Validamos si el código de tipo de marca está informado
		...
		// Validamos si el campo Nombre de tipo de la marca está informado
		if (body.getData().get(MKT_NAM) == null || body.getData().get(MKT_NAM).equals("")) {
			codError = new BigDecimal(20003);
			lvError = lvDlTrnErr.getError(codError, body.getUser().getLanguage().toUpperCase(), "MKT", MKT_NAM, cmpVal);
			error = ValidationError.builder().code(lvError.getErrIdnVal()).field(lvError.getPrpNam()).value(body.getData().get(MKT_NAM)).build();
			errors.add(error);
		}
		...
		if (body.getData().get(DSB_ROW) == null || body.getData().get(DSB_ROW).equals("") || (!body.getData().get(DSB_ROW).equals(CIns.YES) && !body.getData().get(DSB_ROW).equals(CIns.NO))) {
		    ...
		    lvError = lvDlTrnErr.getError(codError, body.getUser().getLanguage().toUpperCase(), "MKT", DSB_ROW,cmpVal);
