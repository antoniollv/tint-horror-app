...
@Service("FdfMngStt")
public class ValidationApiServiceFdfMngStt implements ValidationApiService {

    protected static final String CMP_VAL = "CMP_VAL";

    protected static final String DSB_ROW = "DSB_ROW";

    protected static final String FRD_MNG_STT_NAM = "FRD_MNG_STT_NAM";

    protected static final String FRD_MNG_STT_VAL = "FRD_MNG_STT_VAL";
    ...
    public List<ValidationError> conceptValidation(DataInExtended body) {
		...
		if (accion.equals(CTrn.ACN_TYP_MDF)) {
		    ...
		    // No se puede modificar el Estado de la Gestion del Fraude
		    if(!body.getDataOld().get(FRD_MNG_STT_VAL).equals(body.getData().get(FRD_MNG_STT_VAL))) {
				codError = new BigDecimal(20120);
				lvError = lvDlTrnErr.getError(codError, body.getUser().getLanguage().toUpperCase(), "FDF", FRD_MNG_STT_VAL, cmpVal);
				error = ValidationError.builder().code(lvError.getErrIdnVal()).field(lvError.getPrpNam()).value(body.getDataOld().get(FRD_MNG_STT_VAL)).build();
				errors.add(error);
		    }
		...
		// Validamos  si la descripcion del Estado de la Gestion del Fraude está informado
		if (body.getData().get(FRD_MNG_STT_NAM) == null || body.getData().get(FRD_MNG_STT_NAM).equals("")) {
			codError = new BigDecimal(20003);
			lvError = lvDlTrnErr.getError(codError, body.getUser().getLanguage().toUpperCase(), "FDF", FRD_MNG_STT_NAM, cmpVal);
			error = ValidationError.builder().code(lvError.getErrIdnVal()).field(lvError.getPrpNam()).value(body.getData().get(FRD_MNG_STT_NAM)).build();
			errors.add(error);
		}
