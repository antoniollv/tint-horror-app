...
@Service("FdfCclTyp")
public class ValidationApiServiceFdfCclTyp implements ValidationApiService {

    protected static final String CMP_VAL = "CMP_VAL";

    protected static final String DSB_ROW = "DSB_ROW";

    protected static final String FRD_CCL_TYP_NAM = "FRD_CCL_TYP_NAM";

    protected static final String FRD_CCL_TYP_VAL = "FRD_CCL_TYP_VAL";

    protected static final String FRD_CHC = "FRD_CHC";
    ...
    public List<ValidationError> conceptValidation(DataInExtended body) {
		...
		if (accion.equals(CTrn.ACN_TYP_MDF)) {
		    ...
		    if(!body.getDataOld().get(LNG_VAL).equals(body.getData().get(LNG_VAL))) {
				codError = new BigDecimal(20120);
				lvError = lvDlTrnErr.getError(codError, body.getUser().getLanguage().toUpperCase(), "FDF", LNG_VAL, cmpVal);
		...
		// Validamos  si la descripcion del Tipo de Conclusion del Fraude está informado
		if (body.getData().get(FRD_CCL_TYP_NAM) == null || body.getData().get(FRD_CCL_TYP_NAM).equals("")) {
			codError = new BigDecimal(20003);
			lvError = lvDlTrnErr.getError(codError, body.getUser().getLanguage().toUpperCase(), "FDF", FRD_CCL_TYP_NAM, cmpVal);
			error = ValidationError.builder().code(lvError.getErrIdnVal()).field(lvError.getPrpNam()).value(body.getData().get(FRD_CCL_TYP_NAM).toString()).build();
			errors.add(error);
		}
		...
		// Comprobamos que FRD_CHC valga S o N
		if (body.getData().get(FRD_CHC) == null || (!body.getData().get(FRD_CHC).equals(CIns.YES) && !body.getData().get(FRD_CHC).equals(CIns.NO))) {
		    codError = new BigDecimal(20010);
		    lvError = lvDlTrnErr.getError(codError, body.getUser().getLanguage().toUpperCase(), "FDF", FRD_CHC, cmpVal);
		    error = ValidationError.builder().code(lvError.getErrIdnVal()).field(lvError.getPrpNam()).value(body.getData().get(FRD_CHC).toString()).build();
		    errors.add(error);
		}
