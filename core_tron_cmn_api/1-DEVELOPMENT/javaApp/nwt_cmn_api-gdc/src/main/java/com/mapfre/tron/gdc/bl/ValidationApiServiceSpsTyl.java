...
@Service("SpsTyl")
public class ValidationApiServiceSpsTyl implements ValidationApiService {
	...
	public List<ValidationError> conceptValidation(DataInExtended body) {
		...
		if (body.getData().get("REA_TYL_VAL") == null || (!body.getData().get("REA_TYL_VAL").equals(CIns.YES) && !body.getData().get("REA_TYL_VAL").equals(CIns.NO))) {
			codError = new BigDecimal(20010);
			lvError = lvDlTrnErr.getError(codError, body.getUser().getLanguage().toUpperCase(), "SPS", "REA_TYL_VAL", cmpVal);
			error = ValidationError.builder().code(lvError.getErrIdnVal()).field(lvError.getPrpNam()).value(body.getData().get("REA_TYL_VAL").toString()).build();
			errors.add(error);
		}
