...
@Service("FdfClf")
public class ValidationApiServiceFdfClf implements ValidationApiService {

    protected static final String CMP_VAL = "CMP_VAL";

    protected static final String DSB_ROW = "DSB_ROW";

    protected static final String FRD_CLF_NAM = "FRD_CLF_NAM";

    protected static final String FRD_CLF_VAL = "FRD_CLF_VAL";
    ...
    public List<ValidationError> conceptValidation(DataInExtended body) {
		...
		if (accion.equals(CTrn.ACN_TYP_MDF)) {
		    ...
		    // No se puede modificar la Clasificacion del Fraude
		    if(!body.getDataOld().get(FRD_CLF_VAL).equals(body.getData().get(FRD_CLF_VAL))) {
				codError = new BigDecimal(20120);
				lvError = lvDlTrnErr.getError(codError, body.getUser().getLanguage().toUpperCase(), "FDF", FRD_CLF_VAL, cmpVal);
				error = ValidationError.builder().code(lvError.getErrIdnVal()).field(lvError.getPrpNam()).value(body.getDataOld().get(FRD_CLF_VAL).toString()).build();
				errors.add(error);
		    }
		...
		// Validamos  si la descripcion de la Clasificacion del Fraude está informado
		if (body.getData().get(FRD_CLF_NAM) == null || body.getData().get(FRD_CLF_NAM).equals("")) {
			codError = new BigDecimal(20003);
			lvError = lvDlTrnErr.getError(codError, body.getUser().getLanguage().toUpperCase(), "FDF", FRD_CLF_NAM, cmpVal);
			error = ValidationError.builder().code(lvError.getErrIdnVal()).field(lvError.getPrpNam()).value(body.getData().get(FRD_CLF_NAM).toString()).build();
			errors.add(error);
		}
