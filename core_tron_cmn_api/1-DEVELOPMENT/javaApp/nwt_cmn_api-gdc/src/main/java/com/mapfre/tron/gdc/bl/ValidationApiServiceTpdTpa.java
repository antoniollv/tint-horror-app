...
@Service("TpdTpa")
public class ValidationApiServiceTpdTpa implements ValidationApiService {
    ...
    public List<ValidationError> conceptValidation(DataInExtended body) {
		...
		if (accion.equals(CTrn.ACN_TYP_MDF)) {
			...
			codError = new BigDecimal(20120);
			lvError = lvDlTrnErr.getError(codError, body.getUser().getLanguage().toUpperCase(), "TPD","THP_ACV_VAL", cmpVal);
		...
		// Comprobamos si existe la actividad
			try {
			    if(body.getData().get("COD_ACT_TERCERO") != null  && !(body.getData().get("COD_ACT_TERCERO").equals(""))){
			    lvOTpdAvdS = iDlTpdAvdDAO.get_TpdTpa(cmpVal, new BigDecimal (body.getData().get("COD_ACT_TERCERO")), body.getUser().getLanguage().toUpperCase());
			}
			}
		   ...
		   try {
			if(body.getData().get("COD_RAMO") != null  && !(body.getData().get("COD_RAMO").equals(""))){
            	...
            	// Validamos si el concepto logico
                	try {
                	    if (body.getData().get("COD_CONCEPTO_LOGICO") != null
                		    && !(body.getData().get("COD_CONCEPTO_LOGICO").equals(""))) {
                		iDlTpdTtcDAO.getConceptoLogico(cmpVal, body.getData().get("COD_CONCEPTO_LOGICO").toUpperCase(),
                			body.getUser().getLanguage().toUpperCase());
                	    }
                	} catch (NwtException e) {
                	    error = ValidationError.builder().code(e.getErrors().get(0).getErrIdnVal())
                		    .field(e.getErrors().get(0).getPrpNam()).value(body.getData().get("COD_CONCEPTO_LOGICO")).build();
                	    errors.add(error);
                	}
