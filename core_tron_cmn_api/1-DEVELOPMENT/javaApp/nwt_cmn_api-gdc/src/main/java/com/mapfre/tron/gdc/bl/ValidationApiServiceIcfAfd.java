...
@Service("IcfAfd")
public class ValidationApiServiceIcfAfd implements ValidationApiService {

    protected static final String AFD_ICF_VAL = "AFD_ICF_VAL";
    ...
    public List<ValidationError> conceptValidation(DataInExtended body) {
		...
		if (accion.equals(CTrn.ACN_TYP_MDF)) {
		    if(!body.getDataOld().get(AFD_ICF_VAL).equals(body.getData().get(AFD_ICF_VAL))) {
		    	codError = new BigDecimal(20120);
				lvError = lvDlTrnErr.getError(codError, body.getUser().getLanguage().toUpperCase(), "ICF", AFD_ICF_VAL, cmpVal);
				error = ValidationError.builder().code(lvError.getErrIdnVal()).field(lvError.getPrpNam()).value(body.getDataOld().get(AFD_ICF_VAL).toString()).build();
				errors.add(error);
		    }
