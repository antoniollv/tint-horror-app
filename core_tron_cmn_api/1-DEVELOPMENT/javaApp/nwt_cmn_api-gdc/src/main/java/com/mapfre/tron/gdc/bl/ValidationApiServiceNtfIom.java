...
@Service("NtfIom")
public class ValidationApiServiceNtfIom implements ValidationApiService {
    ...
    public List<ValidationError> conceptValidation(DataInExtended body) {
		...
		// Validacion si tipo de suplemento está informado
		if (body.getData().get("ENR_TYP_VAL") == null || body.getData().get("ENR_TYP_VAL").equals("")) {
			codError = new BigDecimal(20003);
			lvError = lvDlTrnErr.getError(codError, body.getUser().getLanguage().toUpperCase(), "NTF", "ENR_TYP_VAL", cmpVal);
			error = ValidationError.builder().code(lvError.getErrIdnVal()).field(lvError.getPrpNam()).value(body.getData().get("ENR_TYP_VAL")).build();
			errors.add(error);
		}
				
		// Validacion si código de suplemento está informado
		if (body.getData().get("ENR_VAL") == null || body.getData().get("ENR_VAL").equals("")) {
			codError = new BigDecimal(20003);
			lvError = lvDlTrnErr.getError(codError, body.getUser().getLanguage().toUpperCase(), "NTF", "ENR_VAL", cmpVal);
			error = ValidationError.builder().code(lvError.getErrIdnVal()).field(lvError.getPrpNam()).value(body.getData().get("ENR_VAL")).build();
			errors.add(error);
		}
		
		// Validacion si subcódigo de siplemento está informado
		if (body.getData().get("ENR_SBD_VAL") == null || body.getData().get("ENR_SBD_VAL").equals("")) {
			codError = new BigDecimal(20003);
			lvError = lvDlTrnErr.getError(codError, body.getUser().getLanguage().toUpperCase(), "NTF", "ENR_SBD_VAL", cmpVal);
			error = ValidationError.builder().code(lvError.getErrIdnVal()).field(lvError.getPrpNam()).value(body.getData().get("ENR_SBD_VAL")).build();
			errors.add(error);
		}
		...
		// Comprobar si código de suplemento existe
		  try {
		    if((body.getData().get("ENR_VAL") != null  && !(body.getData().get("ENR_VAL").equals(""))) && body.getData().get("ENR_TYP_VAL") != null  && !(body.getData().get("ENR_TYP_VAL").equals(""))
			   && (body.getData().get("ENR_SBD_VAL") != null  && !(body.getData().get("ENR_SBD_VAL").equals(""))) ) {
					     
			String enrTypVal = new String (body.getData().get("ENR_TYP_VAL"));
			BigDecimal enrVal = new BigDecimal (body.getData().get("ENR_VAL"));
			BigDecimal enrSbdVal = new BigDecimal (body.getData().get("ENR_SBD_VAL"));
					    
			lvOTrnNtfS = iDlTrnNtfDAO.get_enrVal(cmpVal, enrVal, enrTypVal, enrSbdVal, body.getUser().getLanguage().toUpperCase()) ;
		    }
		}
		catch (NwtException e) {
			error = ValidationError.builder().code(e.getErrors().get(0).getErrIdnVal().toString()).field(e.getErrors().get(0).getPrpNam()).value(body.getData().get("ENR_VAL").toString()).build();
			errors.add(error);
		}
		  

		  // Comprobar suplemento existe
		  try {
			if((body.getData().get("ENR_VAL") != null  && !(body.getData().get("ENR_VAL").equals(""))) && body.getData().get("ENR_SBD_VAL") != null  && !(body.getData().get("ENR_SBD_VAL").equals(""))
				 && body.getData().get("ENR_TYP_VAL") != null  && !(body.getData().get("ENR_TYP_VAL").equals(""))) {
						     
				String enrTypVal = new String (body.getData().get("ENR_TYP_VAL"));
				BigDecimal enrVal = new BigDecimal (body.getData().get("ENR_VAL"));
				BigDecimal enrSbdVal = new BigDecimal (body.getData().get("ENR_SBD_VAL"));
						    
			    lvOTrnNtfS = iDlTrnNtfDAO.get_enrSbdVal(cmpVal, enrVal, enrTypVal, enrSbdVal, body.getUser().getLanguage().toUpperCase()) ;
			 }
		}
		catch (NwtException e) {
		    	error = ValidationError.builder().code(e.getErrors().get(0).getErrIdnVal().toString()).field(e.getErrors().get(0).getPrpNam()).value(body.getData().get("ENR_VAL").toString()).build();
