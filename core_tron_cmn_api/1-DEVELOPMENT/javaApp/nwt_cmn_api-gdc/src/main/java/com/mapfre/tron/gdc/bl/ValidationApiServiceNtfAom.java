...
@Service("NtfAom")
public class ValidationApiServiceNtfAom implements ValidationApiService {
    ...
    public List<ValidationError> conceptValidation(DataInExtended body) {
		...
		if (accion.equals(CTrn.ACN_TYP_MDF)) {
		 ...
		 // No se puede modificar Clase de asiento
		    if(!body.getDataOld().get("ENC_VAL").equals(body.getData().get("ENC_VAL"))) {
				codError = new BigDecimal(20120);
				lvError = lvDlTrnErr.getError(codError, body.getUser().getLanguage().toUpperCase(), "NTF", "ENC_VAL",cmpVal);
				error = ValidationError.builder().code(lvError.getErrIdnVal()).field(lvError.getPrpNam()).value(body.getDataOld().get("ENC_VAL")).build();
				errors.add(error);
		    }
		...
		// No se puede modificar código de documento
		...
		// Validamos si el clase de asiento está informado
		if (body.getData().get("ENC_VAL") == null || body.getData().get("ENC_VAL").equals("")) {
			codError = new BigDecimal(20003);
			lvError = lvDlTrnErr.getError(codError, body.getUser().getLanguage().toUpperCase(), "NTF", "ENC_VAL", cmpVal);
			error = ValidationError.builder().code(lvError.getErrIdnVal()).field(lvError.getPrpNam()).value(body.getData().get("ENC_VAL")).build();
			errors.add(error);
		}
		...
		// Validamos si el código de documento está informado
		...
		if (body.getData().get("DSB_ROW") == null || (!body.getData().get("DSB_ROW").equals(CIns.YES) && !body.getData().get("DSB_ROW").equals(CIns.NO))) {
		 ...
		 // Comprobamos que la clase de asiento está vigente	
		try {
		    if((body.getData().get("ENC_VAL") != null  && !(body.getData().get("ENC_VAL").equals("")))) {
			String encVal = new String(body.getData().get("ENC_VAL"));
			
			lvOTrnNtfS = iDlTrnNtfDAO.get_encVal(cmpVal, encVal, body.getUser().getLanguage().toUpperCase());
		    }
		}
		 catch (NwtException e) {
			error = ValidationError.builder().code(e.getErrors().get(0).getErrIdnVal().toString()).field(e.getErrors().get(0).getPrpNam()).value(body.getData().get("ENC_VAL").toString()).build();
			errors.add(error);
	    }
