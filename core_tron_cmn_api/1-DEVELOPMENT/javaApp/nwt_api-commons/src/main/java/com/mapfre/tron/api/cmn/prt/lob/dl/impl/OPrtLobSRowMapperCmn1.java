...
/**
 * OPrtLobSRowMapperCmn1
 *
 * @author BRCHARL
 * @since 1.8
 * @version 24 sep. 2019 14:31:33
 *
 */
public class OPrtLobSRowMapperCmn1 implements RowMapper<OPrtLobS> {
    ...
    public OPrtLobS mapRow(ResultSet rs, int rowNum) throws SQLException {
        ...
        oPrtLobS.setIsuAfrNdc(rs.getString("MCA_ANTIFRAUDE_EMISION"));
        oPrtLobS.setCpyAnxInlQtn(rs.getString("MCA_COPIA_ANEXOS_DESDE_PPTO"));        
        oPrtLobS.setAnxMulLng(rs.getString("MCA_MULTI_IDIOMA_ANEXO"));
        oPrtLobS.setSbtDnpPic(rs.getString("MCA_PSYPD"));
        oPrtLobS.setSbtDnpPicPrdNam(rs.getString("NOM_PRG_PSYPD"));
        oPrtLobS.setLssAfrNdc(rs.getString("MCA_ANTIFRAUDE_SINI"));
        oPrtLobS.setAplMrk(rs.getString("MCA_APLICA_MARCAS"));
        oPrtLobS.setNbrYerMrk(rs.getBigDecimal("NUM_ANIOS_MARCAS"));
        oPrtLobS.setChcRnsEtr(rs.getString("MCA_REA_EXTERNO"));
        oPrtLobS.setChcCrtErrIsu(rs.getString("MCA_CREA_ERROR_EMI"));
        oPrtLobS.setSdrIsuDatTypVal(rs.getString("TIP_VAL_FEC_EMISION_ESTANDAR"));
        return oPrtLobS;
    }
